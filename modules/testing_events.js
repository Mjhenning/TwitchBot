...
async function simulateAll(client, config, delayMs = 1500) {
  ...
  simulateFollow(client, config, 'FollowSimUser');               await delay(delayMs);
  simulateRaid(client, config, 'RaidSimUser', 99);               await delay(delayMs);
  simulateAdBreak(client, config, 30, true);                     await delay(delayMs);
